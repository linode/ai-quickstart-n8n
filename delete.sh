...
#   bash <(curl -fsSL https://raw.githubusercontent.com/linode/ai-quickstart-n8n/main/delete.sh) <instance_id>
#
#==============================================================================
...
# Remote repository base URL (for downloading shared tools when running remotely)
...
_setup_required_files() {
    ...
    if [ "$all_exist" = true ]; then
        ...
        for f in "${files[@]}"; do
            curl -fsSL "${TOOLS_RAW_BASE}/$f" -o "${REMOTE_TEMP_DIR}/$f" || { echo "ERROR: Failed to download $f" >&2; exit 1; }
        done
